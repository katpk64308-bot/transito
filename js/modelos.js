...
function addStreetlightEmitter(scene, streetlight, index, lightStride) {
  ...
  for (
    let vertex = 0;
    vertex < poolGeometry.attributes.position.count;
    vertex += 1
  ) {
  ...
  // Reduz a quantidade de luzes locais e mantém brilho visível entre elas.
...
function createSidewalkStreetlights(trackSamples) {
  ...
  const roads = [
    {
      samples: trackSamples.start,
      width: 27,
      step: 18
    },

    {
      samples: trackSamples.outer,
      width: 25,
      step: 22
    },

    {
      samples: trackSamples.shortcut,
      width: ROAD_W_SHORT,
      step: 18
    },

    {
      samples: trackSamples.final,
      width: 27,
      step: 24
    }
  ];
  ...
  roads.forEach(({ samples, width, step }) => {
    ...
    // A calçada começa após a pista e o meio dela fica a
    // uma distância definida da linha central.
    // Deixar uma margem nas pontas evita cruzamentos.
    const sidewalkCenter =
      width / 2 +
      .55 +
      Math.min(width * .25, 4.5) / 2;

    for (
      let index = step;
      index < samples.length - step;
      index += step
    ) {
...
function measureBuildingFootprint(model, config) {
  ...
  measuredModel.position.set(
    0,
    0,
    0
  );

  measuredModel.scale.set(
    config.scale.x,
    config.scale.y,
    config.scale.z
  );

  measuredModel.rotation.set(
    config.rotation.x,
    0,
    config.rotation.z
  );
...
function placeBuildingsBesideRoads(
  trackSamples,
  buildings
) {
  ...
  orderedBuildings.forEach(
    building => {
      ...
      if (nearestRoad) {
        ...
        // Bring distant buildings back to the nearest street while keeping
        // their original side of it. Nearby buildings stay at their
        // authored position.
        if (
          nearestRoad.distance >
          roadClearance + 18
          ...
          const tangentLength =
            Math.hypot(
              nearestRoad.tangentX,
              nearestRoad.tangentZ
            ) || 1;
    ...
    object => {
      ...
      materials.forEach(
        material => {
          ...
          // Renderizar as duas faces dobra o trabalho de fragmentos.
          // Os modelos do cenário são fechados, portanto a face frontal
          // é suficiente.
          material.side =
            THREE.FrontSide;
...
function loadModel(
  ...
  if (extension === 'max') {
    onError(
      new Error(
        'Arquivos .max precisam ser exportados para .obj, .fbx ou .glb.'
      )
    );
  ...
  onError(
    new Error(
      `Formato de modelo não suportado: .${extension}`
    )
  );
}
      ...
      loadModel(
        ...
        model => {
          ...
          const sidewalkStreetlights =
            name === 'poste1'
              ? createSidewalkStreetlights(
                  trackSamples
                )
              : null;

          const instances =
            sidewalkStreetlights ||
            config.instances ||
            [
              {
                position:
        ...
        error =>
          console.error(
            `Não foi possível carregar o modelo ${name}:`,
            error
          )
      );
    }
