...
function animate() {
  ...
  // Mantém o menu leve: não atualiza física, HUD, minimapa nem renderiza
  // enquanto o cenário estiver oculto.
  ...
  if (trafficCollision) state.speed = 0;

  const currentViolation = trafficCollision?.type ||
    (state.foraEstrada ? 'offroad' : (state.contramao ? 'wrong' : null));
  if (currentViolation && currentViolation !== previousViolation) {
    ...
    state.lawAlerts = state.lawAlerts.filter(alert =>
      alert.type !== currentViolation && alert.expiresAt > alertNow
    );
    state.lawAlerts.push({ type: currentViolation, expiresAt: alertNow + 5000 });
  }
  if (currentViolation) state.lawBroken = true;
